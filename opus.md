...
| 운동 | `C:\dev\apps\gym\` | `leftjap/gym` | 루트 | MVP 실사용 중, Sentry 도입 완료, MCP(stdio) 연동, Google 로그인 도입+토큰 제거 완료(2026-03-30) |
| 어학 | `C:\dev\apps\study\` | `leftjap/study` | 루트 | Phase 2 중간, Sentry 도입 완료, Google 로그인 도입+토큰 제거 완료(2026-03-31) |
| 크립토 | `C:\dev\docs\투자 전략\` | `leftjap/docs` | `투자 전략/AGENTS.md` | 문서 수시 갱신 |
| keep | `C:\dev\apps\keep\` | `leftjap/keep` | 루트 | B-55 검증 대기, 로컬 일일 백업 운영 중, Sentry 에러 모니터링 도입 완료, MCP(stdio) 연동, 보안 조치 완료(Gemini+CSE 키 삭제, Code.js 데드 코드 제거, JWT 서명 검증 도입+폴백 토큰 제거, 2026-03-30), 매출처-카테고리 학습 도입(클라이언트, 2026-03-31) |
...
| L-07 | 멀티유저 경로 누락 | keep 인증·동기화·배포 변경 시 양쪽 계정(leftjap, soyoun312) 경로를 모두 시뮬레이션. 체크: ①USER_CONFIG 보호 ②토큰/인증 경로 양쪽 통과 ③iOS 단축어 양쪽 확인 ④테스트에 양쪽 포함 | keep |
...
| B-62 | 작업지시서 범위 이탈 방지 + 세션 시작 흐름 재설계 | 규칙 설계 중 |
